...
#  endif
   ...
   Read_vector(local_x, local_n, n, "x", comm_sz, my_rank, comm);
...
void Read_n(
	...
	if (my_rank == 0) {
		printf("Digite el orden del vector: \n");
		scanf("%d", n_p);
	}
	...
	Check_for_error(local_ok, fname,
			"n tiene que ser > 0 y divisible por comm_sz", comm);
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      for(int proc_dest=1; proc_dest < comm_size; proc_dest++){
    	  MPI_Send(a, n , MPI_DOUBLE, proc_dest, 0, comm);
      }
      free(a);
...
void Print_vector(
	...
	if (my_rank == 0) {
		...
		if (b == NULL)
			local_ok = 0;
		Check_for_error(local_ok, fname, "No se pudo asignar memoria al vector temporal", comm);

		for(int i=0; i<comm_sz; i++){
			MPI_Recv(local_b, local_n, MPI_DOUBLE, 0, 0, comm);
		}
		...
		Check_for_error(local_ok, fname, "No se pudo asignar memoria al vector temporal",
				comm);
